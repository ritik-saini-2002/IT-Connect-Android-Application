package com.saini.ritik.windowscontrol.viewmodel
...
import com.saini.ritik.windowscontrol.network.*
...
class PcControlViewModel(private val context: Context) : ViewModel() {
    ...
    private val _scanResults = MutableStateFlow<List<com.saini.ritik.windowscontrol.network.PcLanScanner.DiscoveredAgent>>(emptyList())
    val scanResults: StateFlow<List<com.saini.ritik.windowscontrol.network.PcLanScanner.DiscoveredAgent>> = _scanResults.asStateFlow()
    ...
    fun startLanScan(durationMs: Long = 4_000L) {
        ...
        scanJob = viewModelScope.launch {
            try {
                val scanner = com.saini.ritik.windowscontrol.network.PcLanScanner(context)
    ...
    fun deleteDevice(device: PcSavedDevice) {
        viewModelScope.launch {
            ...
            com.saini.ritik.windowscontrol.network.PcThumbnailFetcher
    ...
    fun refreshThumbnail(device: PcSavedDevice) {
        viewModelScope.launch {
            val path = com.saini.ritik.windowscontrol.network.PcThumbnailFetcher
    ...
    fun wakePc(device: PcSavedDevice) {
        ...
        if (!com.saini.ritik.windowscontrol.network.WakeOnLan.isValidMac(mac)) {
            _uiState.value = PcUiState.Error("Set a valid MAC address first")
            return
        }
        viewModelScope.launch {
            val result = com.saini.ritik.windowscontrol.network.WakeOnLan
    ...
    fun saveFromScan(
        agent    : com.saini.ritik.windowscontrol.network.PcLanScanner.DiscoveredAgent,
        label    : String,
        secretKey: String,
        isMaster : Boolean
    ) {
     ...
     * See [com.saini.ritik.windowscontrol.network.PcControlApiClient.sendGesture]
