...
class CreateUserActivity : ComponentActivity() {
    ...
    @Inject lateinit var adminTokenProvider: com.saini.ritik.core.AdminTokenProvider
    ...
    private suspend fun loadAdminData() {
        try {
            ...
            val canCreate = profile.role == Permissions.ROLE_SYSTEM_ADMIN
                || Permissions.PERM_CREATE_USER        in sessionPerms
                || Permissions.PERM_ACCESS_ADMIN_PANEL in sessionPerms
            if (!canCreate) {
    ...
    fun createUser(
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                db.userDao().upsert(
                    com.saini.ritik.localdatabase.UserEntity(
                        id                   = userId,
                        name                 = name,
                        email                = email,
                        role                 = role,
