...
class ComplaintRepository {
    ...
    private fun convertToComplaintWithDetails(doc: DocumentSnapshot): ComplaintWithDetails? {
        return try {
            ...
            val createdBy = when (val createdByData = data["createdBy"]) {
                is Map<*, *> -> {
                    UserInfo(
            ...
            val status = when (val statusData = data["status"]) {
            ...
            val createdAt = when (val createdAtTimestamp = data["createdAt"]) {
                is Timestamp -> createdAtTimestamp.toDate().time
                is Long -> createdAtTimestamp
                else -> System.currentTimeMillis()
            }

            val updatedAt = when (val updatedAtTimestamp = data["updatedAt"]) {
                is Timestamp -> updatedAtTimestamp.toDate().time
